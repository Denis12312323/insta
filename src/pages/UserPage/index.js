...
const UserPage = () => {
    ...
    return (
        <Layout nickName={authorizedUser.nickname} id={authorizedUser.id} avatarUrl={authorizedUser.avatarUrl}>
            {isPostsLoading || isUserLoading ? <div className="MainLoaderContainer">
                <Bars color="#000BFF" height={80} width={80} />
            </div> : <div className="UserPageRoot">
                {!isUserError && <UserBio
                ...
                <div className="UserPageRootContent">
                    ...
                    >
                        {postsForRender.map(({ comments, likes, imgUrl, id }) =>
                            ...
                            />)}
                    </InfiniteScroll> : !isPostsError && <p className="UserPageNoPosts">User dont have posts!</p>}
                </div>
            </div>}
        </Layout>
    );
